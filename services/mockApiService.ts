...
import { User, Car, GamificationLevel } from '../types';
...
export const calculateFinancialService = async (serviceType: string, user: User): Promise<SimulationResult> => {
  ...
  const scoreMultiplier = user.gamification.level === GamificationLevel.Platinum ? 0.8 : 1.0; // Better rates for higher levels
  ...
  switch (serviceType) {
    case 'financing':
      ...
      return {
        ...
        details: [
          ...
          { label: 'Parcela Mensal', value: new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(installment) },
        ],
    ...
    case 'consortium':
      ...
      return {
        ...
        details: [
          { label: 'Carta de Crédito', value: new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(letterCredit) },
          { label: 'Taxa de Administração', value: `${adminFee}% (Total)` },
          { label: 'Prazo', value: `${duration} meses` },
          { label: 'Parcela Estimada', value: new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(monthlyConsortium) },
        ],
    ...
    case 'equity':
      ...
      return {
        ...
        details: [
          { label: 'Crédito Disponível', value: new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(equityValue) },
    ...
    case 'insurance':
      ...
      return {
        ...
        details: [
          ...
          { label: 'Valor Anual', value: new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(insuranceValue) },
...
export const checkVehicleStatus = async (type: 'fines' | 'ipva' | 'insurance_status', car: Car): Promise<SimulationResult> => {
  ...
  // Simulate status based on vehicle year (older cars have more issues mock logic)
  ...
  if (type === 'ipva') {
    const ipvaValue = 2500; // Mock
    return {
      title: 'IPVA 2025',
      details: [
        { label: 'Status', value: 'Em aberto' },
        { label: 'Valor Integral', value: new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(ipvaValue) },
